...
export class ZephyrService {
    ...
    constructor(context: vscode.ExtensionContext, jiraService?: JiraService) {
     ...
     */
    private getConfig(): ZephyrConfig {
        ...
        return {
            ...
            projectKey: config.get<string>('projectKey', 'PLTF'),
            projectId: config.get<string>('projectId', '10024'),
    ...
    async findFolderByName(folderName: string): Promise<string> {
        ...
        try {
            ...
            const searchFolders = (folderList: any[]): any => {
            ...
            if (!folder) {
                ...
                const searchFoldersCaseInsensitive = (folderList: any[]): any => {
                    for (const f of folderList) {
            ...
            if (folder) {
                ...
                const choice = await vscode.window.showWarningMessage(
                    `Folder "${folderName}" does not exist in Zephyr. What would you like to do?`,
                    'Create New Folder',
                    'Cancel Upload',
                    'Use Root Folder'
                );
    ...
    async uploadTestCases(
    ...
    ): Promise<{successful: number, failed: number, uploadedTestCases: {key: string, id: number, name: string}[]}> {
        const config = this.getConfig();
        ...
        const uploadedTestCases: {key: string, id: number, name: string}[] = [];
            ...
            try {
                ...
                if (folderId !== 'mock-folder-id' && folderId !== 'root') {
                    ...
                    const getResponse = await axios.get(
                        `${config.baseUrl}/testcases/${createdTestCase.key}`,
                        { headers: this.getHeaders() }
                    );
        ...
        if (failed > 0) {
            vscode.window.showWarningMessage(
                `⚠️ Uploaded ${successful} test cases, ${failed} failed. Check console for details.`
            );
     ...
     */
    private async addTestStepsToTestCase(testCaseKey: string, testSteps: string, expectedResult: string): Promise<void> {
        const config = this.getConfig();
     ...
     */
    private async addTestStepsViaTM4J(testCaseKey: string, testSteps: string, expectedResult: string): Promise<void> {
        const config = this.getConfig();
        ...
        try {
            ...
            const testCaseResponse = await axios.get(
                `${config.baseUrl}/testcases/${testCaseKey}`,
                { headers: this.getHeaders() }
            );
            ...
            const response = await axios.put(
                `${config.tm4jBaseUrl}/testcase/${testCaseId}`,
                testStepData,
                {
                    headers: {
            ...
            if (error.response) {
                ...
                if (error.response.status === 401) {
                    ...
                    vscode.window.showErrorMessage(
                        `❌ JWT token expired or invalid. Get a fresh token from Zephyr and update settings.`
                    );
                }
            }
     ...
     */
    private async addTestStepsViaV2API(testCaseKey: string, testSteps: string, expectedResult: string): Promise<void> {
        const config = this.getConfig();
        ...
        try {
            ...
            const response = await axios.post(
                `${config.baseUrl}/testcases/${testCaseKey}/teststeps`,
                testStepsPayload,
                { headers: this.getHeaders() }
            );
            ...
            vscode.window.showWarningMessage(
                `⚠️ Failed to add test steps. For better results, configure JWT token in settings.`
            );
        }
    }
     ...
     */
    private parseStepsToZephyrFormat(testSteps: string, expectedResult: string): any {
    ...
    async getTestCasesFromFolder(folderName: string): Promise<string[]> {
        ...
        try {
            ...
            return testCases.map((tc: any) => tc.key);
        } catch (error: any) {
    ...
    async linkTestCasesToJira(
        ...
        try {
            ...
            const response = await axios.post(
                `${config.tm4jBaseUrl}/tracelink/bulk/create`,
                traceLinks,
                {
                    headers: {
                        'accept': 'application/json, text/plain, */*',
